...
echo "start server.."
wget --output-document=/dev/null --timeout=100 --tries=1 -q --spider http://localhost:${SERVER_PORT}
is_active=`echo $?`
if [ "${is_active}" = "0" ]; then
...
wget --output-document=/dev/null --timeout=100 --tries=1 -q --spider http://localhost:${DEBUGGER_PORT}
